...
//	if (GlobalAppState::get().s_sensorIdx == 7) {//comment for structuresensor need Uplink change by guan
...
void bundlingThreadFunc() {
	...
	while (1) {
		...
		if (g_bundler->getExitBundlingThread()) {//here is the problem!!!!
...
int main(int argc, char** argv)
{
	...
	try {
		...
		if (argc >= 3) {
			...
			fileNameDescGlobalBundling = "zParametersBundlingDefault.txt";
                ...
                //https://blog.csdn.net/sjhuangx/article/details/50329917?utm_source=blogxgwz8
		...
		if (argc == 4) //for scan net: overwrite .sens file
		{
			...
			if (!out.is_open()) throw MLIB_EXCEPTION("unable to open log file " + util::removeExtensions(filename) + ".friedliver.log");// cann't parse
	...
	catch (...)//https://zhidao.baidu.com/question/362589483369869252.html
	{
